...
public class GunController : MonoBehaviour
{
    ...
    public ParticleSystem muzzleFlashPrefab; // MuzzleEffect prefab
    public Transform muzzleEffectSpawnPoint; // MuzzleEffect'in spawn olacaðý referans noktasý
    ...
    void Update()
    {
        // Fire1'e basýlý tutulduðunda sürekli ateþ et
        if (Input.GetButton("Fire1") && canShoot && !isReloading && Time.time >= nextFireTime)
        {
            if (currentAmmo > 0)
            {
                Shoot();
